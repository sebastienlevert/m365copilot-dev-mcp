...
- Always validate TypeSpec compilation using the `compile_typespec` MCP tool before provisioning.
...
- Add conversation starters with `@conversationStarter(#{ title, text })` to guide users. Limit the number of conversation starters to 6.
...
- **Best practices**:
  - Limit the number of conversation starters to 6.
...
Actions define the operations your agent can perform by calling external APIs. Actions are defined in dedicated namespaces with proper decorators for service metadata, authentication, and operation details.
...
Here's a complete example showing how to define actions for the GitHub API:
...
namespace GitHubAPI {
   ...
   */
  const SERVER_URL = Environment.GITHUB_API_URL;
  ...
  /**
   * Get a specific issue by repository owner, name, and issue number.
   * Use this to retrieve detailed information about a specific issue.
   * 
  ...
  @card(#{
    dataPath: "$",
    file: "adaptiveCards/githubIssue.json",
    properties: #{
      title: "$.title",
      url: "$.html_url"
    }
...
**Key elements:**
- **Imports**: Import required TypeSpec libraries and environment configuration
- **Using statements**: Bring HTTP and Actions namespaces into scope
- **@service**: Marks the namespace as a service definition
- **@server**: Specifies the base URL (referenced from environment variables)
- **@actions**: Provides metadata that describes the API to the LLM and users
...
Actions often require authentication to access external APIs. TypeSpec supports multiple authentication schemes.

#### API Key Authentication

Use `@useAuth` with `ApiKeyAuth` for APIs that require an API key in headers, query parameters, or cookies:
...
**Parameters:**
- First generic parameter: Location of the API key (`ApiKeyLocation.header`, `ApiKeyLocation.query`, or `ApiKeyLocation.cookie`)
- Second generic parameter: Name of the header, query parameter, or cookie

**Best practices:**
- Store API keys in environment variables, never hardcode them
- Use descriptive key names that match the API's documentation
- Document the required permissions/scopes in comments

#### OAuth2 Authentication

Use `@useAuth` with `OAuth2Auth` for APIs that require OAuth2 authorization:
...
**Parameters:**
- **type**: OAuth2 flow type (`authorizationCode`, `implicit`, `password`, or `clientCredentials`)
- **authorizationUrl**: URL for user authorization
- **tokenUrl**: URL for token exchange
- **refreshUrl**: URL for token refresh
- **scopes**: Array of OAuth2 scopes required by the API

**Best practices:**
- Use `authorizationCode` flow for user-delegated access
- Store OAuth URLs in environment variables for flexibility across environments
- Document required scopes clearly in your agent documentation
- Test the OAuth flow thoroughly before deployment
